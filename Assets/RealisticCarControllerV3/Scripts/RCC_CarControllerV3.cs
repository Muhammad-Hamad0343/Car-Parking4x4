...
public class RCC_CarControllerV3 : RCC_Core {
    ...
    public void StartEngineOnStart()
    {
        engineRunning = true;
        fuelInput = 1f;
        Time.timeScale = 1;
    ...
    private void Update() {
        ...
        //Reversing Bool.
        if (brakeInput > .9f && transform.InverseTransformDirection(Rigid.velocity).z < 1f && canGoReverseNow && AutomaticGear && !semiAutomaticGear && !changingGear && direction != -1 && !RCC_InputManager.Instance.logitechHShifterUsed)
            StartCoroutine(ChangeGear(-1));
        else if (throttleInput < .1f && transform.InverseTransformDirection(Rigid.velocity).z > -1f && direction == -1 && !changingGear && AutomaticGear && !semiAutomaticGear && !RCC_InputManager.Instance.logitechHShifterUsed)
    ...
    private IEnumerator HideRandomTextAfterDelay(float delay)       //// Hide the text after a specific duration, use a Coroutine like this:
    {
        yield return new WaitForSeconds(2f);
      
    }

    public void CallLevelFailPanel()
    {

  //      AdsManager.Instance.Show_banner_rectangle_bottom();           //Updated

   //     AdsManager.Instance.HideBannerAd();           //Updated
        Time.timeScale = 0;
    }
    /// <summary>
    ...
    private void OnCollisionEnter(Collision collision) {
        ...
        //    if (this.gameObject.CompareTag("PlayerBus"))
        //    {
        //        if (MainMenuManager.CareerModeSelectionInt == 1 && MainMenuManager.instance.CareerModeBool == true)
        //        {



        ////        AdsManager.Instance.ShowMenuInterstitialAd();                 //Updated
        //        //   GamePlayManager.instance.LevelFailPanel.SetActive(true);
        //        GamePlayManager.instance.LevelFailInfoPanel.SetActive(true);
        //             AudioManager.Instance.PlaySfx("LevelFail");
        //            Invoke("CallLevelFailPanel", 1f);

        //             GamePlayManager.instance.SetRCCVolume(0.4f);
        //             Handheld.Vibrate();

        //             //PlayerPrefs.SetInt("UnlockBus4Ad", 50);     //set value to empty for using bus one time only unlock from ads

        //        //   Time.timeScale = 0;

        //    }
        //        if (GamePlayManager.instance.randomTexts.Length > 0 )
        //        {
             
        //            // Get a random index to select a random text from the array
        //            int randomIndex = UnityEngine.Random.Range(0, GamePlayManager.instance.randomTexts.Length);
        //            string randomText = GamePlayManager.instance.randomTexts[randomIndex];


        //            GamePlayManager.instance.randomTextUI.text = randomText;
        //            GamePlayManager.instance.randomTextUI.gameObject.SetActive(true);
        //            GamePlayManager.instance.AnimatonActive();
        //            GamePlayManager.instance.PlayRandomAudio();
                   

        //            StartCoroutine(HideRandomTextAfterDelay(2f));
        //            if (GamePlayManager.instance.highScores >= 10 && MainMenuManager.CareerModeSelectionInt != 1)
        //            {

        //                GamePlayManager.instance.highScores = GamePlayManager.instance.highScores - 10;
        //                PlayerPrefs.SetInt("highscores", GamePlayManager.instance.highScores);
        //                 StartCoroutine(GamePlayManager.instance.PlayCoinsEffect());
        //            }
        //            if(GamePlayManager.instance.LevelFailPanel.active == true)
        //            {
        //                GamePlayManager.instance.StopRandomSounds();
        //            }
        //            //  PlayerPrefs.GetInt("highscores",GamePlayManager.instance.highScores);
        //            // Display the random text in the console (you can change this to UI text display)

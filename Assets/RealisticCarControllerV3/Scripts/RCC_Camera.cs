...
public class RCC_Camera : MonoBehaviour {
    ...
    private void Update() {
        ...
        if (TPSDynamic) {
            ...
            TPSDynamicInput = cameraTarget.Velocity.magnitude / 100f;
    ...
    //<<<<<<<<<<<<<<<<<<<<<<< To limitize Hood Camera Rotation >>>>>>>>>>>>>>>>>>>>>>>>>>>
    float minXRotation = 0f; // Minimum X rotation limit in degrees (looking down).
    float maxXRotation = 0f;  // Maximum X rotation limit in degrees (looking up).
    float minYRotation = -20f; // Minimum Y rotation limit in degrees.
    float maxYRotation = 20f;  // Maximum Y rotation limit in degrees.
    /// <summary>
    /// FPS mode.
    /// </summary>
    private void FPS() {
        ...
        if (useOrbitInHoodCameraMode)
        {
            // Clamp the orbitY_Smoothed and orbitX_Smoothed values within the defined limits.
            float clampedYRotation = Mathf.Clamp(orbitY_Smoothed, minXRotation, maxXRotation);
            float clampedXRotation = Mathf.Clamp(orbitX_Smoothed, minYRotation, maxYRotation);

            // Apply the clamped rotation.
            transform.rotation = cameraTarget.playerVehicle.transform.rotation * Quaternion.Euler(clampedYRotation, clampedXRotation, 0f);
        }
        else
        {
            transform.rotation = cameraTarget.playerVehicle.transform.rotation;
    ...
    private void TPS() {
        ...
        //  If TPS dynamic is enabled, reduce distance and height related to vehicle speed.
        if (TPSDynamic) {
